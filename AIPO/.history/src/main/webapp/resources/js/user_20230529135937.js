...
const handleCheckItem = (event) => {
    ...
    for (var i = 0; i < trAll.length; i++) {
      var tdItemCheck = trAll[i].querySelector('td[name="checkBoxItem"] input[type="checkbox"]');
      console.log(trAll[i].querySelector(' input[type="checkbox"]'));
  console.log(tdItemCheck);
      if (tdItemCheck &&tdItemCheck.checked) {
        itemDelete.push(inputUserId.value);
      }
    }
  
    var deleteUrl = itemDelete.join(',');
