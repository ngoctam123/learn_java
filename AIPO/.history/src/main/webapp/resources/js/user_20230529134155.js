...
var trAll = document.querySelectorAll('tr[name="trBody"]')

for(var i = 0 ; i < trAll.length; i++) {
    var tdItemCheck = trAll[i].querySelector('td[name="checkBoxItem"]');
    var inputUserId = trAll[i].querySelector('input[name="userId"]');
    
    if(tdItemCheck.checked){
        itemDelete
    }
}

var checked =   document.querySelectorAll('input[name="checkBoxItem"]:checked');
var deleteUrl =  Array.from(checked).map((item) => `${item.value}`).join();

  btnRemove.setAttribute("href", "/AIPO/user/list?delete="+deleteUrl)
};
