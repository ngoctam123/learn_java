  ...
  var checkBoxItemChecked = document.querySelectorAll('input[name="checkBoxItem"]:checked');
...
const handleSelectedSearch  = () => {
    for(var i =0; i <selectedSearch.length;i++){
        if (selectedSearch.options[i].selected) {
            btnSearch.setAttribute("href", "/AIPO/user/list?searchSelectBox="+selectedSearch.options[i].value)
        }
    }
}
...
const handleCheckItem = (event) =>{
    console.log("checkBoxItem.length : "+checkBoxItem.length);
    console.log("checkBoxItemChecked.length : "+checkBoxItemChecked.lengthh);
    var isCheckedAll = checkBoxItem.length == checkBoxItemChecked.length
